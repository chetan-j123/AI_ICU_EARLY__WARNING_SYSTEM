...
// Target ranges for each phase (min, max)
const phaseRanges = {
  normal: {
    heartRate: [65, 95],
    spO2: [95, 99],
...
// Move current value smoothly toward a random point inside the target range
function stepToward(current, [min, max], smoothing = 0.25, noise = 0.4) {
  const target = min + Math.random() * (max - min);
  const next = current + (target - current) * smoothing + (Math.random() - 0.5) * noise;
  return next;
}
...
async function testBackendConnection() {
  ...
  if (connected) {
    ...
    alert('❌ Backend connection failed. Check if Spring Boot and the ML service are running.');
  }
}
...
function updateGauge() {
  ...
  // BRIGHTER COLORS for the needle and center
  const color = state.riskLevel === 'CRITICAL' ? 'hsl(0, 85%, 65%)' : 
                state.riskLevel === 'BORDERLINE' ? 'hsl(45, 100%, 65%)' : 'hsl(120, 100%, 60%)';
  ...
  scoreEl.style.textShadow = `0 0 15px ${color}`;  // increased glow
  labelEl.textContent = state.riskLevel;
  labelEl.style.color = color;
  labelEl.style.textShadow = `0 0 10px ${color}`;   // add glow to text
